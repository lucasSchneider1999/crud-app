...
@section('content')
    ...
    <br>
    @if (isset($vaga))
        <form name="formEdit" id="formEdit" method="post" action="{{ route('vagas.update', $vaga->id) }}">
        @method('PUT')
    @else    
    ...
    @endif
        ...
        <input type="text" name="Descricao" id="Descricao" placeholder="Descricao" value="{{$vaga->Descricao ?? ''}}">
        <button>
            @if (isset($vaga))
